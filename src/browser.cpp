...
void Browser::initialize() {
    ...
    if (AppState::getInstance()->aircraftVariant == VariantZibo738) {
        ...
        refreshButton = new Button(Path::getInstance()->pluginDirectory +
                                       "/assets/icons/refresh-cw.svg",
                                   22, 22);
        refreshButton->setPosition(refreshButton->relativeWidth / 2.0f + 0.5f,
                                   1.00f);
        refreshButton->setClickHandler([]() {
            if (!AppState::getInstance()->browserVisible) {
                return;
            }

            AppState::getInstance()->browser->refresh();
        });
        ...
        backButton = new Button(Path::getInstance()->pluginDirectory +
                                "/assets/icons/arrow-left-circle.svg");
        backButton->setPosition(backButton->relativeWidth / 2.0f + 0.01f,
                                0.967f);
        backButton->setClickHandler([]() {
            if (!AppState::getInstance()->browserVisible) {
                return;
        ...
        refreshButton = new Button(Path::getInstance()->pluginDirectory +
                                       "/assets/icons/refresh-cw.svg",
                                   18, 18);
        refreshButton->setPosition(refreshButton->relativeWidth / 2.0f + 0.56f,
                                   0.97f);
        refreshButton->setClickHandler([]() {
            if (!AppState::getInstance()->browserVisible) {
                return;
    ...
    glTexImage2D(
        GL_TEXTURE_2D,
        0,       // mipmap level
        GL_RGBA, // internal format for the GL to use.  (We could ask for a
                 // floating point tex or 16-bit tex if we were crazy!)
        AppState::getInstance()->tabletDimensions.textureWidth,
        AppState::getInstance()->tabletDimensions.textureHeight,
        0,                // border size
...
void Browser::destroy() {
    if (handler && handler->browserInstance) {
        ...
        auto startTime =
            std::chrono::steady_clock::now() + std::chrono::seconds(99);
        ...
        while (1) {
            ...
            if (!handler->browserInstance &&
                ...
                startTime = std::chrono::steady_clock::now();
            } else if (std::chrono::steady_clock::now() - startTime >
                       gracePeriod) {
                break;
            }
        }
        ...
        // Never call CefShutdown(); since this makes all further
        // CefInitialize(); crash. #if IBM debug("Cleaning up CEF
        // instance...\n"); CefShutdown(); #endif
    }
...
void Browser::update() {
    ...
    if (backButton) {
        backButton->opacity =
            AppState::getInstance()->browserVisible ? 1.0f : 0.0f;
    }

    if (lastGpsUpdateTime > __FLT_EPSILON__ &&
        XPLMGetElapsedTime() > lastGpsUpdateTime + 1.0f) {
        updateGPSLocation();
    }
}
...
void Browser::draw() {
    ...
    glBegin(GL_QUADS);
    float brightness =
        Dataref::getInstance()->getCached<float>("avitab/brightness");
    glColor4f(brightness, brightness, brightness, 1.0f);

    float u =
        (float)tabletDimensions.browserWidth / tabletDimensions.textureWidth;
    float v =
        (float)tabletDimensions.browserHeight / tabletDimensions.textureHeight;
...
bool Browser::click(XPLMMouseStatus status, float normalizedX,
                    float normalizedY) {
    if (!textureId || !handler || !handler->browserInstance) {
        return false;
    }
    ...
    if (status == xplm_MouseDown) {
        handler->browserInstance->GetHost()->SendMouseClickEvent(
            mouseEvent, MBT_LEFT, false, 1);
    } else if (status == xplm_MouseDrag) {
        // Yes, we already send this event in mouseMove(). Adding the line below
        // makes it more responsive.
        handler->browserInstance->GetHost()->SendMouseMoveEvent(mouseEvent,
                                                                false);
    } else {
        handler->browserInstance->GetHost()->SendMouseClickEvent(
            mouseEvent, MBT_LEFT, true, 1);
    }

    return true;
}

void Browser::scroll(float normalizedX, float normalizedY, int clicks,
    ...
    handler->browserInstance->GetHost()->SendMouseWheelEvent(
        mouseEvent, horizontal ? clicks : 0, horizontal ? 0 : clicks);
}
...
void Browser::setFocus(bool focus) {
    ...
    if (!focus && handler->hasInputFocus) {
        ...
        handler->browserInstance->GetMainFrame()->ExecuteJavaScript(
            script, "about:blank", 0);
    }
}

void Browser::key(unsigned char key, unsigned char virtualKey,
                  XPLMKeyFlags flags) {
    if (!textureId || !handler) {
        return;
    }
    ...
    keyEvent.type = (flags == 0 || (flags & xplm_DownFlag) == xplm_DownFlag)
                        ? KEYEVENT_KEYDOWN
                        : KEYEVENT_KEYUP;
...
bool Browser::createBrowser() {
    ...
    browser_settings.windowless_frame_rate =
        AppState::getInstance()->config.framerate;
    ...
    CefString(&settings.resources_dir_path) =
        Path::getInstance()->pluginDirectory + "/win_x64/res";
    CefString(&settings.locales_dir_path) =
        Path::getInstance()->pluginDirectory + "/win_x64/res/locales";
    CefString(&settings.browser_subprocess_path) =
        Path::getInstance()->pluginDirectory + "/win_x64/avitab_cef_helper.exe";
#elif APL
    ...
    CefString(&settings.framework_dir_path) =
        ...
        "/mac_x64/Chromium Embedded Framework.framework";
    CefString(&settings.browser_subprocess_path) =
        Path::getInstance()->pluginDirectory +
        "/mac_x64/cefclient Helper.app/Contents/MacOS/cefclient Helper";
#elif LIN
#endif
    ...
    handler = CefRefPtr<BrowserHandler>(new BrowserHandler(
        textureId, AppState::getInstance()->tabletDimensions.browserWidth,
        AppState::getInstance()->tabletDimensions.browserHeight));
    ...
    bool browserCreated = CefBrowserHost::CreateBrowser(
        window_info, handler,
        !pendingUrl.empty() ? pendingUrl
                            : AppState::getInstance()->config.homepage,
        browser_settings, nullptr, request_context);
    if (!browserCreated) {
        AppState::getInstance()->showNotification(new Notification(
            "Error creating browser",
            "An error occured while starting the browser.\nPlease verify if "
            "there are any updates for the " FRIENDLY_NAME
...
void Browser::updateGPSLocation() {
    ...
    float latitude =
        Dataref::getInstance()->get<float>("sim/flightmodel/position/latitude");
    float longitude = Dataref::getInstance()->get<float>(
        "sim/flightmodel/position/longitude");
    float speedMetersSecond = Dataref::getInstance()->get<float>(
        "sim/flightmodel/position/groundspeed");
    float altitudeMeters =
        Dataref::getInstance()->get<float>("sim/flightmodel/position/y_agl");
    float magneticHeading =
        Dataref::getInstance()->get<float>("sim/flightmodel/position/mag_psi");
    ...
    stream << "longitude: " << std::fixed << std::setprecision(6) << longitude
           << ", ";
    stream << "accuracy: 10, ";
    stream << "altitude: " << std::fixed << std::setprecision(0)
           << altitudeMeters << ", ";
    stream << "altitudeAccuracy: 10, ";
    stream << "heading: " << std::fixed << std::setprecision(0)
           << magneticHeading << ", ";
    stream << "speed: " << std::fixed << std::setprecision(0)
           << speedMetersSecond << ", ";
    stream << "}, timestamp: Date.now() }; for (let key in "
              "window.avitab_watchers) { "
              "window.avitab_watchers[key](window.avitab_location); }";

    handler->browserInstance->GetMainFrame()->ExecuteJavaScript(
        stream.str(), "about:blank", 0);
...
CefMouseEvent Browser::getMouseEvent(float normalizedX, float normalizedY) {
    ...
    mouseEvent.y =
        tabletDimensions.browserHeight *
        (1.0f - ((normalizedY - offsetStart) / (offsetEnd - offsetStart)));
    return mouseEvent;
}
